...
<script>
  ...
  function addMarkers(map, city) {
      ...
      function replaceCity(city) {
        ...
        $(city.restaurants).each(function(index, restaurant) {
          newContent+='<li>'+getRestaurantHTML(restaurant)+'</li>'
        })      
        newContent+='</ol></div>'
      ...
      function getRestaurantHTML(restaurant) {        
        ...
        content+='<p class="restaurant_description">'+restaurant.description+'</p>'
        content+='<p class="restaurant_address">Adresse: '+restaurant.address+'</p>'
        content+='<p class="restaurant_phone">Telefon: '+restaurant.phone+'</p>'
        content+='<p class="restaurant_hours"><u>Öffnungszeiten</u></p>'
        content+='<table class="hours">'        
        $(restaurant.hours).each(function(index, hour){
          content+='<tr><td><p>'+hour.day+'</p></td><td><p>'+hour.timings+'</p></td></tr>'
        });  
        content+='</table>'
        if (restaurant.website) {
          content+='<p class="restaurant_web">Website: <a href="'+restaurant.website+'">'+restaurant.website+'</a></p>'  
        }
        content+='</div>'
        if (restaurant.review_id) {
          content+='<span class="yelp-review" data-review-id="'+restaurant.review_id+'" data-hostname="www.yelp.de"></span>'
        }
        return content
      }
      </script>
